...
> Confirm-dialog mechanism (simple confirm/cancel vs. type-to-confirm) is still an open Design question — see the note at the end of this document. Outline 1 below is written mechanism-agnostic on purpose.
...
Confirm-dialog mechanism for "Leave workspace" (simple confirm/cancel naming the workspace, vs. type-to-confirm) has no design-authoritative answer. The only prior answer (2026-06-10) was role-played, never ratified by real Design, and the available mockup asset does not capture the confirmation modal. Resolve with Design/Dev before parametrizing Outline 1 at the interaction level in Stage 1.

---
_Reconciled 2026-08-05 — see `acceptance-criteria.md` in this folder for full source trail._
